...
# One way to find out API_HOST and AUTH_KEY look in $HOME/.wskprops
...
wsk package update --apihost "$APIHOST" --auth "$AUTH" pushnotifications \
-a description "pushnotifications service" \
-a parameters '[ {"name":"appId", "required":true}, {"name":"appSecret", "required":true}]'

wsk action update --apihost "$APIHOST" --auth "$AUTH"  "webhook.js" \
...
wsk action update --apihost "$APIHOST" --auth "$AUTH"  "sendMessage.js" \
